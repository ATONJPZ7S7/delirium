...
*Setup instructions for a gitian build of DLC using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DLC
...
- [Building DLC](#building-delirium)
...
Clone the git repositories for delirium and gitian and then checkout the delirium version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/delirium-crypto/delirium.git
cd delirium
...
Follow the instructions in [doc/release-process.md](release-process.md) in the delirium repository
...
To build DLC (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the delirium repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/delirium/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/delirium-crypto/delirium
...
For example:
```bash
URL=https://github.com/crowning-/delirium.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit delirium=${COMMIT} --url delirium=${URL} ../delirium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit delirium=${COMMIT} --url delirium=${URL} ../delirium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit delirium=${COMMIT} --url delirium=${URL} ../delirium/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/delirium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/delirium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/delirium-build.assert
```
...
[delirium/gitian.sigs](https://github.com/delirium-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.

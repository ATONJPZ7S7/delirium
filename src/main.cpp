...
#if defined(NDEBUG)
#error "Delirium cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Deliriun Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight+1 == 2) {
      nSubsidy = 1000001 * COIN; // Money supply premine for  pre-venda
    } else if (nHeight+1 == 3) {
      nSubsidy = 500000 * COIN; // staking new chain
    } else if (nHeight+1 >= 4 && nHeight+1 < 417) {
      nSubsidy = 0 * COIN;
...
{
    RenameThread("delirium-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
      if (((block.vtx[0].vout.size() != 1) && (block.vtx[0].vout.size() != 2)) || !block.vtx[0].vout[0].IsEmpty())
       ...
       REJECT_INVALID, "bad-proof-of-stake block";

        // Second transaction must be coinstake, the rest must not be
        if (block.vtx.empty() || !block.vtx[1].IsCoinStake())  
            return state.DoS(100, error("CheckBlock() : second tx is not coinstake"));
        REJECT_INVALID, "bad-coinstake-tx";

        for (unsigned int i = 2; i < block.vtx.size(); i++)
            if (block.vtx[i].IsCoinStake()) 
                return state.DoS(100, error("CheckBlock() : more than one coinstake"));
              REJECT_INVALID, "bad-coinstake-one";

         //additional check against false PoS attack

                // Check for coin age.
                // First try finding the previous transaction in database.a
               CTransaction txPrev;
               uint256 hashBlockPrev;
               if (!GetTransaction(block.vtx[1].vin[0].prevout.hash, txPrev, hashBlockPrev, true))
                 ...
                 REJECT_INVALID, "bad-stake-transaction";
            ...
            // then get the previous block in mapBlockIndex.
                ...
                if (it != mapBlockIndex.end())
                 ...
                 REJECT_INVALID, "bad-block-index";
                if (pindex->GetBlockHeader().nTime + nStakeMinAge > GetAdjustedTime())
                 ...
                 REJECT_INVALID, "bad-age-missing";

                if (!block.vtx[0].IsCoinBase() && chainActive.Tip()->nHeight - pindex->nHeight < STAKE_MIN_CONF)
                        return state.DoS(100, error("CheckBlock() : stake under min. required confirmations"));
                 REJECT_INVALID, "bad-imput-missing", true;

                 if (txPrev.vout[block.vtx[1].vin[0].prevout.n].nValue < Params().StakingMinInput()) {
          		return state.DoS(100, error("CheckBlock() : stake input below minimum value"));
                 REJECT_INVALID, "bad-imput-missing";
                 
                //if (block.nTime > SOFT_FORK_VERSION_TIME1) {
        }
    }
    // ----------- swiftTX transaction scanning -----------
...
int ActiveProtocol()
{
    if (chainActive.Tip()->nHeight >= SOFT_FORK_VERSION1)

...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::DLC)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("DLC")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeDeliriumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDeliriumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeDeliriumAmount = strAnonymizeDeliriumAmount.remove(strAnonymizeDeliriumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDeliriumAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeDeliriumAmount * COIN) nMaxToAnonymize = nAnonymizeDeliriumAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeDeliriumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeDeliriumAmount));
        strAnonymizeDeliriumAmount = strAnonymizeDeliriumAmount.remove(strAnonymizeDeliriumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDeliriumAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
